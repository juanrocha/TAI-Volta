...
legend(x = 6, y = 5,legend = colnames(df[-c(1,2)])[16:19], fill = FlowerCol [16:19], bty = 'n' , title = 'Resource', cex = 1)
...
```{r SM_Fig4, fig.height=2, fig.width=6, fig.cap="PCA results", dev.args=list(pointsize = 5) }
library(FactoMineR)
library(factoextra)
par(mfrow=c(1,3))
pca_1 <- PCA(df[-c(1,2,29)])
barplot(pca_1$eig$eigenvalue, ylab="value", xlab="Eigenvalues", names.arg=as.character(1:26))
```
Similarly to the results presented with non-metric multidimensional scaling (Fig 1), PCA separate two big groups of clusters across the first component with a lot of overlap across the second component. The vectors confirm that green, purple and orange clusters are dominated by agroecosystems whose production focuses on millet, cowpeas, rice and sorghum. The purple and orange clusters have the higher aridity and mean temperatures, but also the higher concentration of water reservoirs and cattle per square kilometer. The orange cluster has the fasters growing population and also the highest variability in crop production as well as the highest concentration of farmers. The blue and yellow clusters in the south tend to be more urbanized, have higher rates of literacy, higher migration (both internal and regional), as well as agroecosystems dominated by the production of yam and maize. The red cluster has particularly high population density and market access.

```{r SM_Fig5, echo=FALSE, fig.cap="Graphical illustration of differences in variables between archetypes based on PCA."}
fviz_pca_biplot(pca_1, habillage = as.factor(df$clus), repel=TRUE, label="var", addEllipses=TRUE, palette = levelCols) + xlab("PC 1 (45.74%)") + ylab("PC 2 (12.39%)") + ggtitle("Ordination with principal component analysis")

```

SM Table 1 shows the loadings of each component. On the first component, variables with high positive values such as aridity, mean temperature, and produced kilocalores of cowpeas, millet or sorghum separate apart the orange, green and purple clusters; while variables with negative values such as wet season, literacy, regional migration and urbanization are responsible for the differentiation of the yellow, blue and red groups. On the vertical axis (teh second component) variables responsible for explaining variation includes population density, ratio of women, market access and production in kilocalories of yam and maize.
